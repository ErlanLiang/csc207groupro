...
public class TetrisBoard implements Serializable{
    ...
    public int placementHeight(TetrisPiece piece, int x) {
        ...
        int[] lowestYVals = new int[width];
        System.arraycopy(piece.getLowestYVals(), 0, lowestYVals, 0, width);
        int maxHeight = 0;
        for (int i = 0; i < width; i++) {
            int colHeight = this.getColumnHeight(x + i);
            colHeights[i] = colHeight;
        ...
        while (flag) {
            for (int i = 0; i < width; i++) {
                if (lowestYVals[i] + maxHeight > colHeights[i]) {
                    lowestYVals[i] -= 1;
                } else {
                    flag = false;
                    break;
    ...
    public int placePiece(TetrisPiece piece, int x, int y) {
        ...
        for (int filled_positions : this.rowCounts) {
            if (filled_positions == this.getWidth()) {
    ...
    public int clearRows() {
        ...
        for (int i = 0; i < maxHeight; i++) {
            if (this.rowCounts[i] == this.getWidth()) {
                ...
                for (int x = 0; x < this.tetrisGrid.length; x++) {
                    for (int y = i; y < maxHeight - 1; y++) {
                        this.tetrisGrid[x][y] = this.tetrisGrid[x][y+1];
                    }
                }
                for (int j = 0; j < this.tetrisGrid.length; j++) {
                    this.tetrisGrid[j][maxHeight - 1] = false;
                }
                this.makeHeightAndWidthArrays();
