...
public class TetrisPiece implements Serializable {
    ...
    */
    ...
    public TetrisPiece(TetrisPoint[] points) {
        TetrisPoint[] tetrisPoints = new TetrisPoint[points.length];
        for (int i = 0; i < points.length; i++) {
            tetrisPoints[i] = new TetrisPoint(points[i]);
        }
        this.body = tetrisPoints;
        HashMap<Integer, Integer> xyMap = new HashMap<>();
        int h = 0;
        for (TetrisPoint point : this.body) {
            if (point.y > h) {
                h = point.y;
            }
            if (!xyMap.containsKey(point.x)) {
                xyMap.put(point.x, point.y);
            } else {
                if (point.y < xyMap.get(point.x)) {
                    xyMap.put(point.x, point.y);
                }
            }
        }
        int[] yVals = new int[xyMap.size()];
        for (int i = 0; i < xyMap.size(); i++) {
            yVals[i] = xyMap.get(i);
        }
        this.lowestYVals = yVals;
        this.height = h + 1;
        this.width = xyMap.size();
    }
    ...
    public boolean equals(Object obj) {
        ...
        return Arrays.equals(other.lowestYVals, this.lowestYVals) &&
                Arrays.equals(other.computeNextRotation().lowestYVals, this.computeNextRotation().lowestYVals);
    }
    ...
    public static TetrisPiece makeFastRotations(TetrisPiece root) {
        TetrisPiece currRoot = root;
        while (!currRoot.computeNextRotation().equals(root)) {
            TetrisPiece temp = currRoot.computeNextRotation();
            currRoot.next = temp;
            currRoot = temp;
        }
        currRoot.next = root;
    ...
    public TetrisPiece computeNextRotation() {
        TetrisPoint[] tetrisPoints = new TetrisPoint[this.body.length];
        for (int i = 0; i < this.body.length; i++) {
            TetrisPoint tp = this.body[i];
            tetrisPoints[i] = new TetrisPoint(tp.y, this.width - tp.x - 1);
        }
        return new TetrisPiece(tetrisPoints);
    }
